...
session = boto3.Session(profile_name='testseven')
# Any clients created from this session will use credentials
# from the [dev] section of ~/.aws/credentials.
client = session.client('devicefarm', region_name='us-west-2')
# client = boto3.client('devicefarm', region_name='us-west-2')
configuration_device_farm = {}

## Create a project
response = client.create_project(
    name='iOS_e2e_Testing',
)

configuration_device_farm.update(response)
...
response = client.create_device_pool(
    ...
    name='iOS_e2e_Testing_Device_Pool',
    description='iOS Devices for e2e-testing',
    # You can get the project ARN by using the list-projects CLI command.
    projectArn=,
    rules=[{'attribute': 'AVAILABILITY',
            'operator': 'IN',
...
configuration_device_farm.update(response)
print(configuration_device_farm)

# response = client.list_projects()
# print(response)
#
# response = client.list_device_pools(arn='arn:aws:devicefarm:us-west-2:806459281259:project:256d49c0-40b3-4f9c-8726-1083c0d7af84')
# print(response)
#
# print(configuration_device_farm)
...
#     joblink = r"https://circleci.com/gh/{0}/{1}/{2}".format(os.environ.get('CIRCLE_PROJECT_USERNAME'),
...
#     response = send_email(title, htmloutput, contentformat=ContentFormat.Html, fromaddress=configure['email_from'],
#                           toaddresses=configure['alarm_email_to'])
#     print(response)
#
# print("done")
